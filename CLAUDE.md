...
- **iOS/Android parity is a hard requirement.** Every current and future feature must work identically on both native platforms, not just "mobile" in the abstract — this repo has a track record of a feature shipping iOS-only (RevenueCat IAP, push notifications, AdMob native IDs) with Android silently no-op'ing until it's audited and closed later. When adding or changing a mobile-facing feature, check **both** `ios/` and `android/` native config, not just the Capacitor JS layer — JS can look platform-neutral (e.g. `src/lib/purchases.ts`) while the underlying native manifest/plist/gradle/xcconfig wiring still isn't. Android-specific config lives alongside its iOS equivalent: `android/app/google-services.json` (gitignored, Firebase/FCM, dropped in manually — see `release/ANDROID_RELEASE_SETUP.md`), `android/key.properties` (gitignored, release signing), `VITE_REVENUECAT_API_KEY_ANDROID` in `.env.mobile` next to the iOS key, `android/variables.gradle`'s `admobAndroidAppId`, and `FirebaseFcmGateway`/`FcmPushService` in `backend/.../service/` next to `PushyApnsGateway`/`ApnsPushService`.
