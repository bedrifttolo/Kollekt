...
- **Push notifications:** client foundation (permission, device-token registration after login, tap deep-links) in `src/lib/pushNotifications.ts`; tokens persisted via `POST /api/push/device-token` (`push_device_tokens` table, one row per device — a member can hold several simultaneously). Delivery: `PushDispatchService` fans each notification out to `ApnsPushService` (iOS, via `PushyApnsGateway`/Pushy) and `FcmPushService` (Android, via `FirebaseFcmGateway`/Firebase Admin SDK); each filters to its own platform's tokens and both silently no-op if their respective config (`APNS_*`/`FCM_CREDENTIALS_BASE64`) is unset, so push delivery degrades gracefully rather than failing loudly where it isn't configured. See `release/ANDROID_RELEASE_SETUP.md` for the Firebase project setup this needs.
- **Realtime is broadcast-per-collective, not per-member:** `CollectiveWebSocketHandler`/`RealtimeUpdateService` key WebSocket sessions by collective (a `Set<WebSocketSession>`), not by member — every device of every member connected to a collective's socket receives every event for that collective, independent of platform. This is what makes a household on a mix of iOS/Android/web devices, and a single member with several simultaneous devices, "just work" without any per-platform or per-device handling — a member with two devices just gets each event twice, once per socket, which the client already handles idempotently via refetch.
